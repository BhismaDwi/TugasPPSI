<?php
	...
	$all = mysql_query("SELECT * FROM db_kunjungan where jam_keluar is null and jam_masuk is not null and status='Dapat Menemui' ORDER BY id_kujungan DESC");
	$count_all = mysql_num_rows($all);
	$query = mysql_query("SELECT * FROM db_kunjungan where jam_keluar is null and jam_masuk is not null and status='Dapat Menemui' ORDER BY id_kujungan DESC LIMIT $offset,10");
	...
	if($count<10){
		if($count==0){
			...
			$query = mysql_query("SELECT * FROM db_kunjungan where jam_keluar is null and jam_masuk is not null and status='Dapat Menemui' ORDER BY id_kujungan DESC LIMIT $offset,$count");
			$count = mysql_num_rows($query);
			if(empty($count)){
				$query = mysql_query("SELECT * FROM db_kunjungan where jam_keluar is null and jam_masuk is not null and status='Dapat Menemui' ORDER BY id_kujungan DESC LIMIT 0,10");
				$num = 0;
			}else{
				$num = $offset;
			}
	...
	if($json_kosong==1){
		$json = '[{ "no": "", "nama": "", "instansi": "", "keperluan": "", "nama": ""}]';
	}else{
		$json .= ']';
	}
	echo $json;
